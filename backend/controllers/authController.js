...
exports.register = async (req, res) => {
  try {
    ...
    if (existingUser && !existingUser.isVerified) {
      ...
      existingUser.examPref = examPref;
...
exports.verifyOTP = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Email verified! Welcome to Vidyarthi Mitra.",
      accessToken,
      user: { _id: user._id, name: user.name, email: user.email, phone: user.phone, examPref: user.examPref },
    });
  } catch (error) {
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email: email.toLowerCase() });
    if (!user)
      return res.json({ success: true, message: "If this email exists, an OTP has been sent.", email });
   ...
   // Mask phone number like +91XXXXXX8901
const maskedPhone = user.phone
  ? user.phone.replace(/(\+?\d{2})\d+(\d{4})/, "$1XXXXXX$2")
  : null;

res.json({
  success: true,
  message: `Password reset OTP sent. Please check your ${otpMethod === "sms" ? "phone" : "email"}.`,
  email,
  maskedPhone,
});
  } catch (error) {
...
exports.updateProfile = async (req, res) => {
  try {
    const { name, phone, examPref } = req.body;
    const updates = {};
