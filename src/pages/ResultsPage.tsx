...
export default function ResultsPage() {
  ...
  useEffect(() => {
    const submitAttempt = async () => {
      // Check if already submitted via sessionStorage flag
      if (sessionStorage.getItem(`submitted_${testId}`)) return;
      ...
      try {
        ...
        await axios.post("http://localhost:5000/api/tests/submit", {
          testTitle: testId,
          score: scoredMarks,
          totalQuestions: totalMarks, // For consistency, show scored / total marks
          timeTaken: result.timeTaken
        }, {
          headers: { Authorization: `Bearer ${token}` }
        });
        
        sessionStorage.setItem(`submitted_${testId}`, "true");
      } catch (err) {
        console.error("Failed to save attempt to backend");
      }
    };
    
    submitAttempt();
  }, [testId, scoredMarks, totalMarks, result?.timeTaken]);
