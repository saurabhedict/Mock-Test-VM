...
export default function TestListPage() {
  ...
  const tests = getAllTests(examId || '');
      ...
      <div className="container py-12">
        ...
        <div className="space-y-6">
          {exam.fullLengthTests.map(flt => {
            const fltTests = fullLengthTests.filter(t => t.testId.includes(flt.testId));
            return (
              <div key={flt.testId}>
                <h3 className="text-lg font-semibold text-foreground mb-3">
                  {flt.testName}
                  <span className="ml-2 text-sm font-normal text-muted-foreground">
                    ({flt.totalQuestions} Qs, {flt.totalMarks} Marks)
                  </span>
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {fltTests.map((test, i) => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: i * 0.05 }}
                      className="rounded-xl border border-primary/20 bg-accent/30 p-5 shadow-card hover:shadow-card-hover transition-all"
                    >
                      <h4 className="font-semibold text-foreground">{test.testName}</h4>
                      <div className="mt-2 flex items-center gap-4 text-sm text-muted-foreground">
                        <span className="flex items-center gap-1"><FileText className="h-3.5 w-3.5" /> {test.totalQuestions} Qs</span>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">{test.subject}</p>
                      <Button
                        size="sm"
                        className="mt-4 gap-1"
                        onClick={() => navigate(`/test/${test.testId}`)}
